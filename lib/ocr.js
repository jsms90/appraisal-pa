...
for(var i in uploads) {
   if(extensions.indexOf(path.extname(uploads[i]))) {
       newFile = uploads[i];
   }
}
   ...
   { apikey: '9df0cfe81888957',
     ...
     file:
      { value: fs.createReadStream(newFile),
        options: { filename: path.basename(newFile) + path.extname(newFile), contentType: null } } } };

request(options, function (error, response, body) {
  if (error) throw new Error(error);

  let text = JSON.parse(body)["ParsedResults"][0]["ParsedText"].split("\n").join("<br>");
